    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-3xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="sticky top-0 bg-white px-6 py-4 border-b border-[#F0E0D8] flex items-center justify-between rounded-t-3xl">
          <div>
            <h2 className="font-display text-xl text-[#4A2C2A] font-bold flex items-center gap-2">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-[#4A2C2A] text-center">Baju</p>
            ...
            {/* Upload Foto Diri */}
            <div className="space-y-2">
              <p className="text-xs font-semibold text-[#4A2C2A] text-center">Foto Kamu</p>
            ...
            {/* Hasil */}
            <div className="space-y-2">
              <p className="text-xs font-semibold text-[#4A2C2A] text-center">Hasil</p>
          ...
          <div className="flex gap-3">
            <button onClick={handleTryOn} disabled={loading || !humanImage}
              className="flex-1 flex items-center justify-center gap-2 bg-[#4A2C2A] text-white py-3 rounded-full font-medium hover:bg-[#C4826A] transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm">
              {loading ? <Loader2 size={16} className="animate-spin" /> : <Sparkles size={16} />}
              {loading ? "Memproses..." : "Coba Sekarang"}
            </button>
            {(humanPreview || result) && (
              <button onClick={() => { setHumanImage(""); setHumanPreview(""); setResult(""); setError(""); }}
                className="flex items-center gap-2 border border-[#E8C4B8] text-[#4A2C2A] px-5 py-3 rounded-full hover:border-[#C4826A] transition-colors text-sm">
