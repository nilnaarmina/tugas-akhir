...
export default function ProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!product) return (
    <div className="min-h-screen bg-[#FAF7F2] flex flex-col items-center justify-center gap-4">
      <p className="text-[#4A2C2A]">Produk tidak ditemukan.</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20">
          ...
          <div className="flex flex-col">
            ...
            {product.colors.length > 0 && (
              <div className="mb-4">
                <p className="text-sm font-medium text-[#4A2C2A] mb-2">Warna: <span className="text-[#C4826A]">{selectedColor}</span></p>
                <div className="flex gap-2 flex-wrap">
                  {product.colors.map((c: string) => (
                    <button key={c} onClick={() => setSelectedColor(c)}
                      className={`px-4 py-1.5 rounded-full text-sm border transition-all ${selectedColor === c ? "bg-[#4A2C2A] text-white border-[#4A2C2A]" : "border-[#E8C4B8] text-[#4A2C2A] hover:border-[#C4826A]"}`}>
                      {c}
                    </button>
                  ))}
                </div>
            ...
            {product.sizes.length > 0 && (
              <div className="mb-6">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-sm font-medium text-[#4A2C2A]">Ukuran: <span className="text-[#C4826A]">{selectedSize}</span></p>
                </div>
                <div className="flex gap-2 flex-wrap mb-3">
                  {product.sizes.map((s: string) => (
                    <button key={s} onClick={() => setSelectedSize(s)}
                      className={`w-12 h-10 rounded-lg text-sm font-medium border transition-all ${selectedSize === s ? "bg-[#4A2C2A] text-white border-[#4A2C2A]" : "border-[#E8C4B8] text-[#4A2C2A] hover:border-[#C4826A]"}`}>
                      {s}
                    </button>
                  ))}
                </div>
                {product.size_chart && Object.keys(product.size_chart).length > 0 && (
                  <div className="bg-[#FAF7F2] rounded-xl overflow-hidden border border-[#E8C4B8]">
                    <p className="text-xs font-semibold text-[#4A2C2A] px-4 py-2 border-b border-[#E8C4B8]">Panduan Ukuran</p>
                      ...
                      <tbody>
                        {product.sizes.map((s: string) => product.size_chart[s] && (
                          <tr key={s} className={`border-t border-[#E8C4B8] ${selectedSize === s ? "bg-[#E8C4B8]/40 font-semibold" : ""}`}>
                            <td className="px-4 py-2 text-[#4A2C2A]">{s}</td>
                            <td className="px-4 py-2 text-[#4A2C2A]">{product.size_chart[s].panjang}</td>
                            <td className="px-4 py-2 text-[#4A2C2A]">{product.size_chart[s].lebar}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            <div className="flex gap-3">
              <button onClick={handleAdd}
                className={`flex-1 py-4 rounded-full font-medium flex items-center justify-center gap-2 transition-all duration-300 text-sm ${added ? "bg-green-500 text-white" : "bg-[#4A2C2A] text-white hover:bg-[#C4826A]"}`}>
                <ShoppingBag size={18} />
                {added ? "Berhasil Ditambahkan!" : "Add to Cart"}
              </button>
              <button onClick={() => toggleWishlist(product.id)}
                aria-label={wishlisted ? "Hapus dari wishlist" : "Tambah ke wishlist"}
                className={`w-14 h-14 rounded-full border-2 flex items-center justify-center transition-all ${wishlisted ? "bg-[#C4826A] border-[#C4826A] text-white" : "border-[#E8C4B8] text-[#4A2C2A] hover:border-[#C4826A]"}`}>
